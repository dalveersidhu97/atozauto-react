...
const RadioInput: FC<{ onChange: (value: string) => any, name: string, options: { label: string, value: string }[], defaultValue: string }> = ({ onChange, options, name, defaultValue }) => {
    const inputIdPrefix = useId();
...
export const Preference: FC = () => {
    ...
    const [hotSecondsLessThan, setHotSecondsLessThan] = useState(10);
    const [minutesIncrementor, setMinutesIncrementor] = useState(1);
    const [secondsIncrementorTmp, setSecondsIncrementorTmp] = useState(0);
    const secondsIncrementor = secondsIncrementorTmp+1;

    const [refreshMode, setRefreshMode] = useState('Smart');
    ...
    useEffect(() => {
        console.log('minutesMultiplier', minutesMultiplier)
    }, [minutesMultiplier])
    useEffect(() => {
        console.log('hotSecondsLessThan', hotSecondsLessThan)
    }, [hotSecondsLessThan])
    useEffect(() => {
        console.log('minutesIncrementor', minutesIncrementor)
    }, [minutesIncrementor])
    useEffect(() => {
        console.log('secondsIncrementor', secondsIncrementor)
    }, [secondsIncrementor]);
    useEffect(() => {
    ...
    const hotMinsMultDesc = `Hot Mins: ${[0,1,2,3, 4, 5, 6, 7, 8].map(v=>v*minutesMultiplier).filter(v=>v<61).map(v=>':'+padWithZero(v)).join(',')}`;
            ...
            <div className="flex flex-col gap-4">
                ...
                {refreshMode==='Smart' && <div className="flex flex-col gap-4 border rounded-md p-4">
                    ...
                    <div className="grid grid-cols-2 gap-y-4 gap-x-2">
                        ...
                        <div className="flex flex-col gap-2">
                            <NumberInput min={1} max={60} label="Refresh for first " unit="Seconds" desc={`When its a hot minute`} value={hotSecondsLessThan} onChange={(val) => setHotSecondsLessThan(val)} />
                        </div>
                        <div className="flex flex-col gap-2">
                            <NumberInput min={0} max={59} label="Refresh after every" unit="Seconds" desc="When its a hot minute" value={secondsIncrementorTmp} onChange={(val) => setSecondsIncrementorTmp(val)} />
                        </div>
                        <div className="flex flex-col gap-2">
                            <NumberInput min={1} max={60} label="Refresh after every " unit="Minutes" desc="After hot minute is over" value={minutesIncrementor} onChange={(val) => setMinutesIncrementor(val)} />
                        </div>
                    </div>
                </div>}
            </div>
